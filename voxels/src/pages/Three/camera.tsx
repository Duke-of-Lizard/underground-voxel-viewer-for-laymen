...
export const CameraViewTransition = forwardRef(function CameraViewTransition(props, ref) {
  ...
  const onBeforeToggleCallback = useCallback(
    (manager, targetCamera) => {
      if (!manager.animating) {
        ...
        if (tiles) {
          ...
          tiles.ellipsoid
            .getPositionToNormal(downVector, downVector)
            .multiplyScalar(-1)
        ...
        if (targetCamera.isOrthographicCamera) {
          ...
          targetCamera.matrixWorld.decompose(targetCamera.position, targetCamera.quaternion, targetCamera.scale)
